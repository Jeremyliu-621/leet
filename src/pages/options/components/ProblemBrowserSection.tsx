...
import type { Difficulty, SolvedProblemRecord } from '../../../lib/types';
...
export function ProblemBrowserSection() {
  ...
  useEffect(() => {
    if (!isOpen) return;
    let mounted = true;
    void getValue('solvedProblems').then((records: readonly SolvedProblemRecord[]) => {
  ...
  const filtered =
    diffFilter === 'all'
      ? ALL_PROBLEMS
      : ALL_PROBLEMS.filter((p) => p.difficulty === diffFilter);
