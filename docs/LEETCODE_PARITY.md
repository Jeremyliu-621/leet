...
today versus what LeetMeow's challenge screen ships, focused exclusively on the
...
out-of-scope by design — LeetMeow isn't trying to *be* LeetCode, it's trying to be a
...
| Feature                                  | What LeetCode does                                                                                  | LeetMeow today                              | Status | Priority | Notes                                                                                                                    |
| ---------------------------------------- | --------------------------------------------------------------------------------------------------- | ------------------------------------------- | :----: | :------: | ------------------------------------------------------------------------------------------------------------------------ |
...
| Feature                                  | What LeetCode does                                                                                  | LeetMeow today                              | Status | Priority | Notes                                                                                                                    |
| ---------------------------------------- | --------------------------------------------------------------------------------------------------- | ------------------------------------------- | :----: | :------: | ------------------------------------------------------------------------------------------------------------------------ |
| Title                                    | "#125. Valid Palindrome" with number                                                                | Title string                                |   ✅   |    —     | LeetMeow owns its bank, so no real "problem number" is needed — could still show a stable ID.                            |
| Difficulty pill                          | Colored pill (Easy=green, Medium=yellow, Hard=red)                                                  | Grayscale pill, contrast-graded             |   ✅   |    —     | LeetMeow's grayscale is *intentional* design. Counts as parity.                                                           |
...
| Hints (collapsible)                      | "Show Hint 1 / 2 / 3" buttons; each reveals a paragraph                                             | Progressive reveal with 60s timer cost; 3 hints per problem in the bank | ✅ | — | The friction is intentional — aligned with the LeetMeow brand.                                                           |
| Related topics list                      | Bottom of page: list of related topic tags linking to the topic browser                             | Tag pill row only                           |   🟡   |    L     | Less useful in LeetMeow since there's no topic browser to navigate to.                                                    |
...
| Feature                                  | What LeetCode does                                                                                  | LeetMeow today                              | Status | Priority | Notes                                                                                                                    |
| ---------------------------------------- | --------------------------------------------------------------------------------------------------- | ------------------------------------------- | :----: | :------: | ------------------------------------------------------------------------------------------------------------------------ |
| Console panel at bottom                  | Tabbed panel ("Testcase", "Test Result") below editor                                               | Inline verdict region above the action bar  |   🟡   |    —     | LeetMeow's layout is single-column-of-info — simpler, scans well. Conscious choice.                                       |
...
| Side-by-side expected/actual diff        | Stacked, not side-by-side; just three labelled rows                                                 | Stacked rows ("input" / "expected" / "actual") | ✅ | — | LeetMeow now shows input alongside expected/actual.                                                                      |
| Per-test breakdown row                   | Tabs across the top of result panel — one per test case                                             | One card per test                           |   ✅   |    —     | LeetMeow's *list-of-cards* is arguably more scannable than LC's per-test tabs.                                            |
...
| Verdict states                           | Accepted / Wrong Answer / Runtime Error / TLE / Compile Error / Memory Limit Exceeded               | Accepted / Wrong Answer / Runtime Error / Compile Error / **Timeout (TLE)** | ✅ | — | LeetMeow has every state that matters for our worker-based judge.                                                         |
| Accept banner ("All test cases passed!") | Green banner + confetti                                                                             | Grayscale "accepted" banner                 |   ✅   |    —     | LeetMeow omits confetti by design — solving is the reward, not the celebration.                                            |
| Time-limit-exceeded handling             | Server-side TLE                                                                                     | `worker.terminate()` + `timeout` outcome    |   ✅   |    —     | LeetMeow's hard 4s/6s timeout is honest and visible.                                                                      |
...
| Keyboard-shortcut palette (Cmd+K style)  | None                                                                                                | `?` button → keyboard-shortcuts modal (all shortcuts, Esc/backdrop close) | ✅ | — | LeetMeow ships this; LC doesn't.                                                                                          |
...
| Timer in header                          | Optional timer with auto-reset + "super alarm"                                                       | Always-on MM:SS countdown                   |   ✅   |    —     | LeetMeow's timer is *load-bearing* — it's not optional. Parity++.                                                          |
...
| Feature                                  | What LeetCode does                                                                                  | LeetMeow today                              | Status | Priority | Notes                                                                                                                    |
| ---------------------------------------- | --------------------------------------------------------------------------------------------------- | ------------------------------------------- | :----: | :------: | ------------------------------------------------------------------------------------------------------------------------ |
| Narrow viewport behavior                 | Mobile is essentially unusable; tablet falls back to stacked layout                                 | Stacks below `lg` (~1024 px) into vertical layout |   ✅   |    —     | LeetMeow is responsive *better* than LC on tablets.                                                                       |
| Keyboard navigation through controls     | Partial — Monaco traps focus aggressively                                                           | All buttons have `onKeyDown` handlers for Enter/Space; ARIA labels everywhere |   ✅   |    —     | LeetMeow has the better accessibility story today.                                                                        |
| Focus management on verdict update       | None — verdict appears silently                                                                     | `role="status"` + `aria-live="polite"` on the verdict panel |   ✅   |    —     | LeetMeow screen-reader-announces every run result. Concrete a11y win.                                                     |
| ARIA labels on icon buttons              | Some                                                                                                | All controls have `aria-label`              |   ✅   |    —     |                                                                                                                          |
| Color-contrast on difficulty/verdict     | Color-coded (red/yellow/green) — fails WCAG AA for some color-blind users                           | Grayscale, contrast-graded                  |   ✅   |    —     | LeetMeow is meaningfully better here.                                                                                     |
...
## Where LeetMeow is *better* than LeetCode (for the README / marketing site)
...
4. **Honest TLE.** LeetCode's runtime numbers are noisy and gamified ("0 ms beats 100%"
   is a meme). LeetMeow has a hard 4-second / 6-second worker timeout. If your code
   doesn't terminate, you see it immediately.
5. **No login required.** LeetCode requires an account to save progress. LeetMeow keeps
...
6. **No "show solution" temptation.** LeetCode dangles a Solution tab and a Discussion
   tab on every problem. LeetMeow has no escape hatch. If you wanted to bail, you can
   just close the tab — and that's the cost.
7. **Truly responsive.** LeetMeow stacks cleanly on tablet widths; LC effectively
   requires a desktop.
