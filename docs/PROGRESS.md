...
**Current focus:** Bank at **2792** problems; 8670 tests green. Batches 102–214 + 212-addendum + 207b + KaTeX UX + a11y fix + total-solved UX complete.
**Build status:** 🟢 `npm run typecheck` + `npm run test` green.
**Next up:** More bank growth (batch 215+); UX polish per LEETCODE_PARITY.md.

### feat(bank): batch 214 — count-artifacts-that-can-be-extracted, closest-dessert-cost, watering-plants-ii, rearrange-words-in-a-sentence (2026-05-29)
Four new medium problems: `count-artifacts-that-can-be-extracted` (medium/arrays+hash-map+simulation, Set-based cell lookup for rectangle coverage), `closest-dessert-cost` (medium/dynamic-programming+arrays+backtracking, DFS over toppings with pruning + tie-breaking), `watering-plants-ii` (medium/arrays+two-pointers+simulation, two-pointer meeting-in-middle with per-person refill tracking), `rearrange-words-in-a-sentence` (medium/strings+simulation, stable sort by word length + capitalize). Bank at **2792**; 8670 tests.
