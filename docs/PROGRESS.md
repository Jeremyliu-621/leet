...
**Current focus:** Bank at **2390** problems; tests green. Batches 102–161 (both tracks) complete. UX + a11y polish ongoing.
**Build status:** 🟢 `npm run typecheck` + `npm run test` green.
**Next up:** More bank growth (batch 162+); UX polish.

### feat(bank): add batch 161 — pyramid-transition-numbers (medium), painting-a-grid-with-three-different-colors (hard), maximum-spending-after-buying-items (hard), number-of-good-binary-strings (medium) (2026-05-29)
Four new problems: `pyramid-transition-numbers` (medium/backtracking, DFS build pyramid rows bottom-up, prune when no allowed transitions for a pair), `painting-a-grid-with-three-different-colors` (hard/dynamic-programming, column-pattern DP with bitmask state: enumerate valid column configs, precompute compatibility pairs, DP over n columns), `maximum-spending-after-buying-items` (hard/arrays+heap, greedy: sort all items ascending and assign day multipliers 1..m*n; maximize total by buying cheapest items earliest), `number-of-good-binary-strings` (medium/dynamic-programming, dp[i] = ways to form string of length i via +zero or +one; sum dp[low..high]). JS and Python reference solutions included.

### feat(bank): add batch 161 (other track) — frog-jump-ii, collecting-chocolates, partitioning-min-deci-binary, where-will-ball-fall, split-string-max-unique (2026-05-29)
Five new medium problems: `frog-jump-ii` (medium/arrays+binary-search, two frogs alternate stones — max gap = max of every-other-stone diff), `collecting-chocolates` (medium/arrays+simulation, O(n²) rotation DP with rolling minCost array), `partitioning-into-minimum-number-of-deci-binary-numbers` (medium/strings+math, trivial greedy — answer = max digit), `where-will-the-ball-fall` (medium/arrays+simulation, stuck if nj OOB or grid[r][nj]≠direction), `split-a-string-into-the-maximum-number-of-unique-substrings` (medium/strings+backtracking, DFS with used-set maximize count). JS + Python reference solutions added.

### feat(bank): add batch 160 (other track) — strictly-palindromic-number, design-bitset, count-substrings-with-fixed-ratio, number-of-integers-with-even-digit-sum (2026-05-29)
Four new problems: `strictly-palindromic-number` (medium/math, always returns false since n in base n-2 is always "12" — never palindrome), `design-bitset` (medium/design, flip-flag O(1) ops with stored-vs-logical inversion; fix tracked by ones count), `count-substrings-with-fixed-ratio` (medium/strings+hash-map, prefix key = zeros×num2 − ones×num1 hash map), `number-of-integers-with-even-digit-sum` (easy/math, O(1): num//2 if digitSum(num) even else (num-1)//2).
