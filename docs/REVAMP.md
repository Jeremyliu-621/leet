...
### Phase A — Editor correctness (the concrete complaints) ✅ priority
- [ ] Tab / Shift-Tab indent **selections** (indentMore/indentLess), not replace.
- [ ] Visible, blinking 2px caret; cursor sits above bracket-match highlight.
- [ ] Kill the "green box" — matching brackets get a subtle, boxless highlight.
- [ ] Richer, verified syntax highlighting (the palette is wired; make it pop).
- [ ] Native shortcuts audit (multi-cursor, line move, etc. — keep, document).

### Phase B — LeetCode-like layout
- [ ] Move **Run / Submit** to a centered pill group in the top bar (icons).
- [ ] Panels become rounded "floating" cards with a gutter from screen edges.
- [ ] Editor header: language dropdown (rounded) + icon actions, "Code" label.
- [ ] Slim bottom status bar: Ln/Col, saved, shortcuts.

### Phase C — Terminal parity (screenshot 3)
- [ ] Testcase / Test Result tabs; "Last Executed Input" cards (l1 = [2,4,3]).
- [ ] Color verdicts (green pass / red fail) instead of grayscale-only.

### Phase D — AI Hint Bot (Gemini) — "revolutionary"
- [ ] Settings field for Gemini API key (chrome.storage.sync; NEVER in repo).
- [ ] `src/lib/ai/gemini.ts` client (model: gemini-2.0-flash) — typed, tested.
- [ ] Bot reads problem + user code, returns line-anchored hints.
- [ ] Editor decorations: highlight referenced lines + inline gutter/annotation.
- [ ] Hint panel UI: progressive, non-spoiler, "explain my bug" / "nudge me".
